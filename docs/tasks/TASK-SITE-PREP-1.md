...
> 🔴 **갱신 — SITE-PREP-2 (2026-09-15).** 아래 빈칸 표는 **그때의 기록**이다. 지금은 빈칸 0건이고
> 매장 섹션은 통째로 빠졌다. 이관 당일 절차는 [TASK-SITE-CUTOVER-R1.md](TASK-SITE-CUTOVER-R1.md).
>
> 🔴 **홈페이지에 쓰지 않는 문장 (소유자 확정 2026-09-15)** — 스토어 설명의
> 「이 앱은 인터넷 권한을 사용하지 않습니다」. Billing 연동(sera-saju `25ba1e40`)으로 앱에
> INTERNET 권한이 들어가 **사실과 다르다.** 홈페이지는 「고객 정보는 기기 밖으로 나가지
> 않습니다 · 회사 서버로 전송되지 않습니다」까지만 쓴다. 스토어 설명을 옮겨 올 때 이 문장을 빼고 옮긴다.
